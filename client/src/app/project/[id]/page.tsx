...
export default function ProjectPage() {
  ...
  const parsedCardsPreview = useCallback(() => {
    ...
    let cSep = '\n';
    if (cardSeparator === 'newline') cSep = '\n';
    if (cardSeparator === 'semicolon') cSep = ';';
    if (cardSeparator === 'custom') {
       cSep = customCardSep.replace(/\\n/g, '\n'); // handle literal \n
    }
    ...
    let blocks = [];
    if (cSep === '\n') {
       blocks = importText.split(/\r?\n/);
    } else {
       blocks = importText.split(cSep);
    }
  ...
  const handleImportSubmit = async () => {
    ...
    if (cardsToCreate.length === 0) {
      alert('Không tìm thấy thẻ hợp lệ nào để nhập.');
      return;
    }
    setIsParsing(true);
    try {
      // Bulk create cards (in parallel for MVP simplicity)
  ...
  if (loading) {
    return <div className="flex items-center justify-center min-h-screen text-slate-500">Đang tải dữ liệu...</div>;
  }

  if (!project) {
    return <div className="flex items-center justify-center min-h-screen text-red-500">Không tìm thấy dự án</div>;
  }

  return (
    <div className="container mx-auto p-8 max-w-6xl">
      <Link href="/" className="inline-flex items-center text-slate-500 hover:text-slate-900 dark:hover:text-white mb-6 transition-colors">
        <ArrowLeft className="h-4 w-4 mr-2" /> Quay lại danh sách
      </Link>

      <div className="mb-8 bg-white dark:bg-slate-950 p-6 md:p-8 rounded-2xl border shadow-sm">
        <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-slate-900 dark:text-white">{project.title}</h1>
        {project.description && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Left Column: Form / Parser */}
        <div className="lg:col-span-1">
          <div className="sticky top-8">
            <h2 className="text-xl font-semibold mb-4 text-slate-900 dark:text-white">Thêm thẻ ghi nhớ</h2>
                  ...
                  <CardContent className="pt-6">
                    <form onSubmit={handleManualAdd} className="space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Textarea 
                          id="definition" 
                          placeholder="Bộ xử lý trung tâm..." 
                          value={definition}
                        ...
                        <DialogContent className="max-w-5xl h-[85vh] flex flex-col p-0 gap-0 overflow-hidden">
                          <DialogHeader className="p-6 pb-4 border-b shrink-0 flex flex-row items-center justify-between">
                            <DialogTitle className="text-xl">
                              Nhập dữ liệu <span className="text-sm font-normal text-slate-500 ml-2">Chép và dán dữ liệu ở đây (từ Word, Excel, Google Docs, v.v.)</span>
                            </DialogTitle>
                          </DialogHeader>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                              <div className="space-y-4">
                                ...
                                <RadioGroup value={termSeparator} onValueChange={setTermSeparator}>
                                  <div className="flex items-center space-x-2">
                                    <RadioGroupItem value="tab" id="t-tab" />
                                    <Label htmlFor="t-tab" className="font-normal cursor-pointer">Tab</Label>
                                  </div>
                                  <div className="flex items-center space-x-2">
                                    <RadioGroupItem value="comma" id="t-comma" />
                                    <Label htmlFor="t-comma" className="font-normal cursor-pointer">Phẩy</Label>
                                  </div>
                                  <div className="flex items-center space-x-2 h-10">
                                    ...
                                    {termSeparator === 'custom' && (
                                      <Input value={customTermSep} onChange={e => setCustomTermSep(e.target.value)} className="w-20 h-8" />
                                    )}
                                  </div>
                                </RadioGroup>
                              </div>
                              ...
                              <div className="space-y-4">
                                ...
                                <RadioGroup value={cardSeparator} onValueChange={setCardSeparator}>
                                  <div className="flex items-center space-x-2">
                                    <RadioGroupItem value="newline" id="c-nl" />
                                    <Label htmlFor="c-nl" className="font-normal cursor-pointer">Dòng mới</Label>
                                  </div>
                                  <div className="flex items-center space-x-2">
                                    <RadioGroupItem value="semicolon" id="c-semi" />
                                    <Label htmlFor="c-semi" className="font-normal cursor-pointer">Chấm phẩy</Label>
                                  </div>
                                  <div className="flex items-center space-x-2 h-10">
                                    ...
                                    {cardSeparator === 'custom' && (
                                      <Input value={customCardSep} onChange={e => setCustomCardSep(e.target.value)} className="w-20 h-8" />
                                    )}
                                  </div>
                                </RadioGroup>
                              </div>
                            ...
                            <div className="mt-4 border-t pt-6">
                              ...
                              {parsedCardsPreview().length === 0 ? (
                                ...
                                <div className="space-y-2 max-h-60 overflow-y-auto pr-2">
                                  ...
                                  {parsedCardsPreview().length > 50 && (
                                    <p className="text-center text-sm text-slate-500 py-2">... và {parsedCardsPreview().length - 50} thẻ nữa</p>
                                  )}
                                </div>
                              )}
                            </div>
        ...
        <div className="lg:col-span-2">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-xl font-semibold text-slate-900 dark:text-white">Thẻ của bạn ({flashcards.length})</h2>
            {flashcards.length > 0 && (
              <div className="flex gap-2">
                <Button render={<Link href={`/project/${projectId}/study`} />} nativeButton={false} className="bg-indigo-600 hover:bg-indigo-700 text-white">
                  <BrainCircuit className="h-4 w-4 mr-2" /> Chế độ học
                </Button>
                <Button render={<Link href={`/project/${projectId}/quiz`} />} nativeButton={false} variant="outline" className="border-indigo-200 dark:border-indigo-800 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30">
          ...
          {flashcards.length === 0 ? (
            <div className="text-center py-20 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-dashed border-slate-300 dark:border-slate-800">
              ...
              <h3 className="text-lg font-medium text-slate-900 dark:text-white">Chưa có thẻ nào</h3>
              <p className="text-slate-500 mt-1 max-w-sm mx-auto">Thêm thẻ thủ công ở bên trái hoặc dùng tab nhập hàng loạt để sao chép văn bản nhanh chóng.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {flashcards.map((card, index) => (
                <Card key={card.id} className="group transition-all hover:shadow-md border-slate-200 dark:border-slate-800 overflow-hidden">
                  <div className="flex flex-col md:flex-row">
                    <div className="bg-slate-50 dark:bg-slate-900 flex md:flex-col items-center justify-center p-3 md:w-16 border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-800 text-slate-400 font-bold text-lg">
                      {index + 1}
                    </div>
                    <div className="p-5 flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 relative">
                      <div>
                        <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Thuật ngữ</div>
                        <div className="text-lg font-medium text-slate-900 dark:text-white">{card.term}</div>
                      </div>
                      <div>
                        <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Định nghĩa</div>
                        <div className="text-slate-700 dark:text-slate-300 leading-relaxed">{card.definition}</div>
                      </div>
                      
                      <Button 
                        variant="destructive" 
                        size="icon" 
                        className="absolute top-4 right-4 h-8 w-8 opacity-0 group-hover:opacity-100 transition-opacity shadow-sm"
