...
import { api, Folder, Flashcard } from '@/lib/api';
...
export default function FolderQuizModePage() {
  ...
  const loadData = async () => {
    try {
      ...
      const [folderData, cardsData] = await Promise.all([
        ...
        api.getFolderFlashcards(folderId)
      ]);
  ...
  const generateQuiz = (cards: Flashcard[]) => {
    // Need at least 4 cards total to have 3 distinct distractors plus the correct answer.
    // If fewer than 4 cards, we'll still generate options from what we have, but it might be fewer than 4 options.
    ...
    let generated: Question[] = cards.map(card => {
      ...
      return {
        ...
        options: Array.from(new Set(shuffledOptions)) // ensure uniqueness just in case
      };
    });
  ...
  if (!folder || questions.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen text-center p-4">
        ...
        <p className="text-slate-500 mb-6">Vui lòng thêm ít nhất 1 thẻ ghi nhớ vào thư mục.</p>
        <Button render={<Link href={`/folder/${folderId}`} />} nativeButton={false} variant="outline">
          Quay lại thư mục
        </Button>
      </div>
    );
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 p-6 md:p-12">
        <div className="max-w-3xl mx-auto space-y-8">
          <div className="flex items-center gap-4">
            <Link href={`/folder/${folderId}`} className="text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors bg-slate-200/50 dark:bg-slate-800 p-2 rounded-full">
              <ArrowLeft className="h-5 w-5" />
            </Link>
            <h1 className="text-2xl font-bold">Kết quả: {folder.title}</h1>
          </div>
          ...
          <Card className="p-8 md:p-12 flex flex-col items-center justify-center text-center space-y-6">
            ...
            <div className="flex gap-4 mt-8">
              ...
              <Button render={<Link href={`/folder/${folderId}`} />} nativeButton={false} variant="outline" size="lg">
                Về trang thư mục
              </Button>
            </div>
          </Card>
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col">
      ...
      <header className="bg-white dark:bg-slate-900 border-b px-6 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-4">
          <Link href={`/folder/${folderId}`} className="text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors bg-slate-100 dark:bg-slate-800 p-2 rounded-full">
            <ArrowLeft className="h-5 w-5" />
          </Link>
          <div>
            <h1 className="font-bold text-lg md:text-xl line-clamp-1">{folder.title}</h1>
            <p className="text-xs text-slate-500 font-medium tracking-wide uppercase">Trắc nghiệm Thư mục</p>
          </div>
        </div>
