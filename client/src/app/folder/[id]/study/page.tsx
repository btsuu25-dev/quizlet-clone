...
export default function FolderStudyModePage() {
  ...
  if (!folder || flashcards.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen text-center p-4">
        <h2 className="text-2xl font-bold mb-4">Không có thẻ nào trong thư mục này</h2>
        <p className="text-slate-500 mb-6">Thêm một số học phần vào thư mục để bắt đầu ôn tập.</p>
        <Link href={`/folder/${folderId}`}>
          <Button variant="outline" nativeButton={false}>Quay lại thư mục</Button>
        </Link>
      </div>
    );
  }
      ...
      <header className="bg-white dark:bg-slate-900 border-b px-6 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium tracking-wide uppercase">Chế độ học Thư mục</p>
          </div>
        </div>
      ...
      <main className="flex-1 container mx-auto p-4 md:p-8 flex flex-col items-center justify-center max-w-4xl">
        ...
        <div className="w-full aspect-[4/3] md:aspect-[2/1] relative group perspective-1000" style={{ perspective: '1000px' }}>
          ...
          >
            ...
            <div 
              className="absolute inset-0 w-full h-full bg-blue-50 dark:bg-blue-950/30 border border-blue-100 dark:border-blue-900 shadow-xl rounded-3xl flex flex-col items-center justify-center p-8 md:p-12"
              style={{ backfaceVisibility: 'hidden', transform: 'rotateX(180deg)' }}
            >
              <div className="absolute top-6 left-6 text-sm font-semibold text-blue-400 uppercase tracking-widest">
                Định nghĩa
              </div>
        ...
        <div className="w-full mt-10 flex flex-col items-center gap-6">
          <div className="flex items-center gap-8 bg-white dark:bg-slate-900 p-2 rounded-full shadow-sm border border-slate-200 dark:border-slate-800">
            <Button 
              variant="ghost" 
              size="icon" 
