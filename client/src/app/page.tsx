...
import { Plus, BookOpen, Trash2, LogOut, User as UserIcon, FolderPlus, FileText, Folder as FolderIcon, Home, BrainCircuit } from 'lucide-react';
...
export default function HomePage() {
  ...
  const [isOpen, setIsOpen] = useState(false); // Study Set dialog
  const [isFolderOpen, setIsFolderOpen] = useState(false); // Folder dialog
  ...
  const handleCreateFolder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewFolderTitle('');
      checkAuthAndLoad();
      setActiveTab('library'); // Chuyển sang tab thư viện để xem thư mục vừa tạo
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      checkAuthAndLoad();
      setActiveTab('home'); // Chuyển sang tab trang chủ để xem học phần vừa tạo
    } catch (error) {
      console.error(error);
    }
  };
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger render={
                <Button size="lg" className="gap-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-md shadow-indigo-600/20 transition-all hover:-translate-y-0.5">
            ...
            <Dialog open={isOpen} onOpenChange={setIsOpen}>
              <DialogContent className="sm:max-w-[600px] p-6 sm:p-8 rounded-2xl">
                <form onSubmit={handleCreate}>
                  <DialogHeader className="mb-4">
                    ...
                    <DialogDescription className="text-base text-slate-500 mt-2">
                      Đặt một tiêu đề rõ ràng cho bộ thẻ của bạn.
                    </DialogDescription>
                  </DialogHeader>
                  <div className="grid gap-6 py-4">
                    <div className="grid gap-3">
                      ...
                      <Input 
                        ...
                        autoFocus
                        className="h-14 text-lg bg-slate-50 border-slate-200 focus:bg-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 rounded-xl px-4 shadow-sm transition-all dark:bg-slate-900 dark:border-slate-800 dark:focus:bg-slate-950 font-medium placeholder:text-slate-400"
                      />
                    </div>
                    <div className="grid gap-3">
              ...
              <DialogContent className="sm:max-w-[600px] p-6 sm:p-8 rounded-2xl">
                <form onSubmit={handleCreateFolder}>
                  <DialogHeader className="mb-4">
                    ...
                    <DialogDescription className="text-base text-slate-500 mt-2">
                      Thư mục giúp bạn tổ chức các học phần gọn gàng hơn.
                    </DialogDescription>
                  </DialogHeader>
                  <div className="grid gap-6 py-4">
                    <div className="grid gap-3">
                      ...
                      <Input 
                        id="folderTitle" 
                        placeholder="Ví dụ: Tiếng Anh học kỳ 1" 
            ...
            <div>
              {projects.length === 0 ? (
                <div className="text-center py-24 bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-slate-300 dark:border-slate-800 shadow-sm">
                  ...
                  <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Chưa có học phần nào</h3>
                  <p className="text-slate-500 mb-8 max-w-sm mx-auto">Tạo học phần đầu tiên để lưu trữ các câu hỏi trắc nghiệm của riêng bạn.</p>
            ...
            <div>
              {folders.length === 0 ? (
                <div className="text-center py-24 bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-slate-300 dark:border-slate-800 shadow-sm">
                  ...
                  <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Thư viện trống</h3>
                  <p className="text-slate-500 mb-8 max-w-sm mx-auto">Thư mục giúp bạn nhóm các học phần liên quan lại với nhau để dễ dàng quản lý.</p>
                    ...
                    <Link href={`/folder/${folder.id}`} key={folder.id}>
                      <Card className="h-full hover:shadow-xl transition-all cursor-pointer group relative hover:-translate-y-1.5 bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 overflow-hidden rounded-2xl">
                        ...
                        <CardHeader className="flex flex-row items-center gap-4 pb-4">
                          <div className="p-3.5 bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 rounded-xl shadow-sm">
                            <FolderIcon className="h-7 w-7" />
                          </div>
                          <div className="flex-1 overflow-hidden">
                            <CardTitle className="text-xl font-bold truncate">{folder.title}</CardTitle>
